...
class config:
    confusion_path =  "data/custom_confusion.txt"  # 混淆集
    word_dict_path = "data/dict.txt"  # 字典（包括词频）
    same_pinyin_path = "data/same_pinyin.txt"  # 同音同调与同音异调
    same_stroke_path = "data/same_stroke.txt"  # 同形字
    lm_model_path = "data/people_chars_lm.klm"  # 语言模型路径
    char_set_path = "data/common_char_set.txt"  # 通用字集
    pinyin2word_path = "data/pinyin2word.model"  # 加载拼音转文字模型


class Corrector(object):

    def __init__(self, cut_word_kind):
        ...
        self.cut_word_kind = cut_word_kind  # 分词类型 

        self.confusion_dict = self._load_confusion_dict(self.confusion_path)  # 混淆集字典
        self.word_dict = self._load_word_dict(self.word_dict_path)  # 字典
        self.same_pinyin_dict = self._load_same_pinyin_dict(self.same_pinyin_path)  # 同音字字典
        self.same_stroke_dict = self._load_same_stroke_dict(self.same_stroke_path)  # 形似字字典
        self.pinyin2word = self._load_pinyin_2_word(self.pinyin2word_path)  # 拼音转字
        self.char_set = self._load_char_set(self.char_set_path)  # 字符集合
        self.tokenizer = jieba  # jieba分词
        self.lm = kenlm.Model(self.lm_model_path)  # 语言模型
    ...
    def BMM(self, word_dict, token, window_size=5):
        idxs = []
        result = []
        index = len(token)
        text_size = len(token)
    ...
    def RMM(self, word_dict, token, window_size=5):
        idxs = []
        result = []
        index = len(token)
        flag =0
        window_start = (index - window_size)if index>window_size else 0
        while window_start <= index:
            for size in range(window_start, index, 1):
                piece = token[size:index]
                if piece in word_dict:
                    index = size
                    window_start = (window_start - (index - size)) if (window_start - (index - size))>0 else 0
                    flag = 1
                    idxs.append(size)
                    result.append(piece)
                    break
...
# """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    ...
    def _detect_by_confusion(self, maybe_errors, sentence, start_idx):
        ...
        if self.cut_word_kind == 'FMM':
            idxs, confuses = self.FMM(self.confusion_dict, sentence, max_len)
        elif self.cut_word_kind == 'BMM':
            idxs, confuses = self.BMM(self.confusion_dict, sentence, max_len)

        if len(idxs) > 0:
            for idx, confuse in zip(idxs, confuses):
                maybe_err = [confuse, idx + start_idx, idx + len(confuse) + start_idx, ErrorType.confusion, DetectType.confusion]
    ...
    def _detect_by_token(self, maybe_errors, sentence, start_idx):
        ...
        for token, begin_idx, end_idx in tokens:
            ...
            # 跳过滤空、英文、数字、字母和数字和标点符号
            if _is_filter_token(token):
                continue
            ...
            maybe_err = [token, begin_idx + start_idx, end_idx + start_idx, ErrorType.word, DetectType.token]
            ...
            if ngram_avg_scores:
                ...
                for i in self._get_maybe_error_index(sent_scores, threshold=1):
                    ...
                    if len(token) == 1:
                        ...
                        type = ErrorType.word
                    maybe_err = [token, i + start_idx, i + len(token) + start_idx, type, DetectType.word_ngram]
                    if maybe_err not in maybe_errors and not self._check_in_errors(maybe_errors, maybe_err):
                        maybe_errors.append(maybe_err)
        ...
        try:
            ...
            if ngram_avg_scores:
                ...
                for i in self._get_maybe_error_index(sent_scores):
                    token = sentence[i]
                    maybe_err = [token, i + start_idx, i + len(token) + start_idx, ErrorType.char, DetectType.char_ngram]
                    if maybe_err not in maybe_errors and not self._check_in_errors(maybe_errors, maybe_err):
                        maybe_errors.append(maybe_err)

        except IndexError as ie:
            logger.warn("index error, sentence:" + setence + str(ie))
        except Exception as e:
            logger.warn("detect error, sentence:" + sentence + str(e))
...
# """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""'"""""""""""
    ...
    def _calibration(self, maybe_errors):
        """
        疑似错误列表校准
        :pam mabe_errors: 疑似错误列表
        :retrun: res, 校准后疑似错误列表
        """
        res = []
        pre_item = None
        for cur_item, begin_idx, end_idx, err_type, detect_type in maybe_errors:

            if pre_item is None:
                pre_item = [cur_item, begin_idx, end_idx, err_type, detect_type]
                res.append(pre_item)
                continue
            if ErrorType.char == err_type and err_type == pre_item[3] and begin_idx == pre_item[2]:
                pre_item = [pre_item[0]+cur_item, pre_item[1], end_idx, ErrorType.word, detect_type]
                res.pop()
            else:
                pre_item = [cur_item, begin_idx, end_idx, err_type, detect_type]
            res.append(pre_item)
    ...
    def correct(self, text):
        ...
        text_new = ''
        details = []
        text = self._process_text(text)  # 统一编码
        blocks = split_long_text(text, include_symbol=True)  # 长句分割
        
        for blk, idx in blocks:  # 获得句子和句子
            maybe_errors = self._detect_short(blk, idx)  # 检测
            maybe_errors = self._calibration(maybe_errors)  # 校准
            ...
            for cur_item, begin_idx, end_idx, err_type, detect_type in maybe_errors:
                ...
                if err_type == ErrorType.confusion:
                    ...
                    corrected_item = self.get_lm_correct_item(cur_item, candidates, before_sent, after_sent)  # 获取所有候选项中最为正确的项

                # 对错误项进行纠正
                if corrected_item != cur_item:
                    blk = before_sent + corrected_item + after_sent
                    detail_word = [cur_item, corrected_item, begin_idx, end_idx, err_type, detect_type]

                    details.append(detail_word)

            text_new += blk

        details = sorted(details, key=operator.itemgetter(2))  # 对修正项根据起始下标进行排序
